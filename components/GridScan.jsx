...
export default function GridScan({
  ...
  const skewScaleRef = useRef(0.1);
  const tiltScaleRef = useRef(0.2);
  const yawScaleRef = useRef(0.15);
  const depthResponseRef = useRef(0.35);
  const smoothTimeRef = useRef(0.2);
  const yBoostRef = useRef(1.4);
  ...
  useEffect(() => {
    ...
    // Clear container to avoid "existing context" conflicts during HMR or rapid re-renders
    ...
    const tick = () => {
      ...
      lookCurrent.current.copy(
        smoothDampVec2(lookCurrent.current, lookTarget.current, lookVel.current, smoothTimeRef.current, Infinity, dt)
      );
      ...
      const skew = new THREE.Vector2(lookCurrent.current.x * skewScaleRef.current, -lookCurrent.current.y * yBoostRef.current * skewScaleRef.current);
    ...
    return () => {
      ...
      // Explicitly dispose of everything
  ...
  return (
    <div ref={containerRef} className={`gridscan${className ? ` ${className}` : ''}`} style={style}>
      {webglError && (
        <div className="absolute inset-0 flex items-center justify-center bg-[#060714] text-slate-500 text-xs p-4 text-center z-50">
          <div className="max-w-[200px]">
            <p className="mb-2">Graphics acceleration {webglError === 'lost' ? 'lost' : 'restricted'}.</p>
            <button 
              onClick={() => window.location.reload()}
              className="px-3 py-1 bg-gold-500/20 hover:bg-gold-500/40 text-gold-400 rounded-md transition-colors"
            >
              Reload Page
            </button>
          </div>
